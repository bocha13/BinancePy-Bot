...
last_14_klines = client.get_historical_klines(
    TRADE_SYMBOL, "1m", "3 hours ago UTC", limit=196
)
        ...
        if is_candle_closed:
            ...
            if len(closes) > RSI_PERIOD:
                ...
                if insights[-1] == "sell" and len(np_closes) - intersections[-1] == 2:
                    if in_position:
                        if float(last_buy_price) * min_percentage_profit / 100 + float(
                            last_buy_price
                        ) <= float(close):
...
conn_key = bsm.start_kline_socket(
    TRADE_SYMBOL, process_message, interval=KLINE_INTERVAL_1MINUTE
)
bsm.start()